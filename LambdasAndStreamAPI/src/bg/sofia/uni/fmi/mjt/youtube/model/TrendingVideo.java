...
public final class TrendingVideo {
	...
	private TrendingVideo(String id, String title, LocalDate publishDate, Set<String> tags, long views, long likes,
			long dislikes) {
		this.id = id;
		this.title = title;
	...
	public static TrendingVideo createTrendingVideo(String line) {
		...
		String parsedDate = tokens[PUBLISH_DATE]
				.substring(0, tokens[PUBLISH_DATE].indexOf('T')); // 2017-11-13T17:13:01.000Z
		LocalDate publishDate = LocalDate.parse(parsedDate, DateTimeFormatter.ofPattern("yyyy-MM-dd"));

		Set<String> tags = Stream.of(tokens[TAGS].split("\\|")).map(s -> s.replace("\"", ""))
